...
extern "C"
{
    SWIGEXPORT unsigned char* SWIGSTDCALL ByteVectorData(void* ptr) {
        return reinterpret_cast<std::vector<unsigned char>*>(ptr)->data();
    }
    SWIGEXPORT int SWIGSTDCALL ByteVectorSize(void* ptr) {
       return static_cast<int>(reinterpret_cast<std::vector<unsigned char>*>(ptr)->size());
    }
...
%apply int { int32_t }
%apply unsigned int { uint32_t, size_t }
%apply unsigned long long { uint64_t }
%apply long long { int64_t, libcdoc::result_t }
...
%pragma(csharp) imclasscode=%{
  [DllImport("$dllimport", EntryPoint="ByteVectorData")]
  public static extern IntPtr ByteVectorData(IntPtr data);
  [DllImport("$dllimport", EntryPoint="ByteVectorSize")]
  public static extern int ByteVectorSize(IntPtr data);
  [DllImport("$dllimport", EntryPoint="ByteVectorFree")]
  public static extern void ByteVectorFree(IntPtr data);
  [DllImport("$dllimport", EntryPoint="ByteVectorTo")]
  public static extern IntPtr ByteVectorTo([MarshalAs(UnmanagedType.LPArray)] byte[] data, int size);
%}

%typemap(cstype) std::vector<uint8_t> "byte[]"
%typemap(cstype) uint64_t "ulong"

%typemap(csin, pre= "    IntPtr cPtr$csinput = CDocPINVOKE.ByteVectorTo($csinput, $csinput.Length);
    var handleRef$csinput = new HandleRef(this, cPtr$csinput);"
...
%typemap(csout, excode=SWIGEXCODE) std::vector<uint8_t> {
    ...
    Marshal.Copy($modulePINVOKE.ByteVectorData(cPtr), result, 0, result.Length);
...
// TODO: write typemap for std::vector<std::string> and for std::map<std::string, std::string>.
...
// %apply unsigned char INPUT[] { const uint8_t *src }
%apply unsigned char OUTPUT[] { uint8_t *dst }

%ignore libcdoc::DataConsumer::write(const uint8_t *src, size_t size);
